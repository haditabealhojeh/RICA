...
%  - Optional local refinement using Manopt's conjugategradient (if 'opts.problem' provided)
...
%
% Example:
%  opts = struct(); opts.num_countries = 100; opts.num_iterations = 400;
%  [x, c, h] = rica_solver_v_3(M, @(X) mycost(X), opts);
...
defaults = struct(...
    ...
    'assimilation_coef', 0.3, ...            % legacy single value (kept if provided)
    'assimilation_coef_base', 0.3, ...     % adaptive base (larger initial moves)
    'assimilation_alpha', 1.0, ...          % annealing exponent (lower -> longer big steps)
    'assimilation_gamma', pi/4, ...
    'revolution_rate0', 0.2, ...           % initial revolution probability
    'revolution_tau', [], ...               % will set based on iterations if empty
    'num_iterations', 800, ...
    'seed', 42, ...
    'verbosity', 1, ...
    'empire_assignment', 'power', ...    % 'random','power','roulette','round_robin'
    'empire_competition', 'probabilistic', ... % 'none','classic','tournament','probabilistic','aggressive'
    'assimilation_mode', 'stochastic', ...    % 'geodesic','stochastic','momentum'
    'use_momentum', false, ...
    'momentum_beta', 0.85, ...
    'prob_swap', true, ...                 % probabilistic imperialist swap (simulated annealing)
    'swap_T0', 1.0, ...
    'swap_tau', [], ...
    'local_refine', true, ...               % requires opts.problem to be set (Manopt problem struct)
    'refine_every', 50, ...
    'refine_iters', 5, ...
    'refine_dirs' , 8, ...
    ...
    'refine_decay' , 0.6, ...
    'refine_topk' , 5, ...
    'levy_revolution_prob', 0, ...       % chance for long jump inside levy_revolution
...
% optional: small global population pool updated each iter (imperialists + colonies)
% will be built each iteration

% prepare optional local refinement
has_problem_for_refine = isfield(opts, 'problem') && ~isempty(opts.problem) && isstruct(opts.problem);
...
for iter = 1:opts.num_iterations
    ...
    % Lightweight local refinement for top-k imperialists
    if opts.local_refine && has_problem_for_refine && mod(iter, opts.refine_every) == 0
        try
            smallprob = opts.problem;
            smallprob.cost = @(X) cost_func(X);
            options = struct('maxiter', min(opts.refine_iters, 30), 'verbosity', 0);
            % refine only top 1 imperialists (or fewer)
            [~, rank_idx] = sort(imperialist_costs);
            topk = min(1, numel(rank_idx));
            for r = 1:topk
                ii = rank_idx(r);
                x0 = imperialists{ii};
                try
                    % use Manopt conjugategradient if available
                    [x_ref, cval, ~, ~] = conjugategradient(smallprob, x0, options);
                    if cval < imperialist_costs(ii)
                        imperialists{ii} = x_ref;
                        imperialist_costs(ii) = cval;
                    end
                catch
            ...
            % ignore if conjugategradient not available
        end
    end
