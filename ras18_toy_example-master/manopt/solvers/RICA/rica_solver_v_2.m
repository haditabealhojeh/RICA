function [best_sol, best_cost, conv_history, all_countries_out] = rica_solver_v_2(M, cost_func, opts, X0)
...
% X0:       [NEW] Optional initial solution (warm start)
    % --- Default parameters ---
    if nargin < 3
        opts = struct();
    end
    
    % NEW: Check if a warm start matrix was passed
    has_warm_start = (nargin >= 4) && ~isempty(X0);
    ...
    for i = 1:opts.num_countries
        if has_warm_start && i == 1
            % Inject the warm start solution (nearestSPD result)
            countries{i} = X0;
        else
