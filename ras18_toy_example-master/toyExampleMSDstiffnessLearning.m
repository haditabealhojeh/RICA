...
load([dataPath 'data_01.mat']);
addpath(genpath('F:\RICA\metaheuristic-manifold-optimization-main (1)\metaheuristic-manifold-optimization-main\manifolds'));
addpath(genpath('F:\RICA\metaheuristic-manifold-optimization-main (1)\metaheuristic-manifold-optimization-main\gmm_gmr'));

%% Problem parameters
windowSize   = 3;           % sliding window size
convexOptim  = 0;           % Do CVX for stiffness estimations
spdApproxim  = 0;           % Do nearestSPD for stiffness estimations
ricaOptim    = 0;           % Do RICA for stiffness estimations (Requires Manopt and rica_solver_v_2)
mDTMAoptim   = 1;           % Do mDTMA for stiffness estimations (NEW)
...
if ~LOAD_TOY_EX_DATA
    ...
    for j=1:n_s_total
        ...
        for i = 1 : nbData
            ...
            % Always compute the basic least squares estimate
            % Always compute the basic least squares estimate first
            Korig = (XtX + ww) \ XtY;
            eeigU(:,i,j) = eig(Korig);
            ...
            elseif (convexOptim)
                ...
                minimize( norm(XtX * Kcvx - XtY, 2));
                Kcvx >= eye(d);
                cvx_end
                KpCVX(:,:,i,j) = Kcvx;      
                K = Kcvx;
            elseif (ricaOptim)
                % RICA-based stiffness estimation
                cost_func = @(K_mat) norm(XtX * K_mat - XtY, 2)^2;
                M = sympositivedefinitefactory(2);
                rica_opts = struct(...
                'num_countries', 50, ...
                'num_imperialists', 5, ...
                'num_iterations', 100, ...
                'assimilation_coef', 0.3, ...
                'revolution_rate', 0.4, ...
                'verbosity', 0);
                rica_opts.empire_assignment = 'roulette';  
    
                rica_opts.empire_competition = 'probabilistic';  
            
                rica_opts.colony_update = 'stochastic';  
                [K_rica, ~, ~] = rica_solver_v_2(M, cost_func, rica_opts);
                K = K_rica;
                dD(j).KP_rica(:,:,i) = K;
            elseif (mDTMAoptim)  % NEW mDTMA OPTION
                % mDTMA-based stiffness estimation on SPD manifold
                problem.M = sympositivedefinitefactory(2);
                problem.cost = @(K_mat) norm(XtX * K_mat - XtY, 2)^2;
                
                % mDTMA parameters
                pn = 50;        % Population size
                itmax = 100;    % Maximum iterations
                w0 = 0.4;       % Initial step size parameter
                
                % Run mDTMA optimization
                [K_mdtma, fbest, info] = mDTMA(problem, pn, itmax,w0,[]);
                
                K = K_mdtma;
                dD(j).KP_mdtma(:,:,i) = K;
            else
                % Use the raw least squares estimate
                K = Korig;
            end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
reproduction=figure('position',[1000,500,500,200]);hold on;
...
xlim([-27, (nbData-rdp)+10]); ylim([-27, 27]);
...
ylabel('$\mathbf{K}^p$', 'Interpreter', 'Latex', 'Fontsize', 10);
axis equal;xlim([-25, 75]); ylim([-25, 25]);
set(gca, 'FontSize', 10,'TickLabelInterpreter','latex');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ylabel('$\mathbf{K}^p$', 'Interpreter', 'Latex', 'Fontsize', 10);
xlabel('$t$', 'Fontsize', 10, 'Interpreter', 'Latex');
xlim([-27, xIn(end)*sc+10]); ylim([-27, 27]);
set(gca, 'FontSize', 10,'TickLabelInterpreter','latex');
...
print(reproduction,'-depsc2','-r300',['fig_reproduction']);
%% Plotting resulting model and reproductions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j = reprosMan
...
ylim([0, max(max(model_series))+2*max(max(model_error))]);
set(ax, 'Fontsize', 12)
